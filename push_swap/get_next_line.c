...
static int	error_free(char *remain, char *buf, int i)
{
	if (remain != NULL)
	{
		free(remain);
...
static int	check_n(char **remain, char **line)
{
	char	*pointer;
...
static int	rest2(char **remain, char **line)
{
...
static int	rest(char **remain, char **line, char *buf, int fd)
{
	int		l;
	int		k;
	int		byte_readed;
